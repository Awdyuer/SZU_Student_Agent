...
/* 学生端现在是「自注册 + 课程码加入」：
   ...
   课程码来自 lesson-data/join-codes.json。
   每次跑用随机学号 —— 账户在服务端持久化，写死学号第二次跑就会 409。
*/
const joinCodes = JSON.parse(readFileSync(
  new URL("../../../lesson-data/join-codes.json", import.meta.url),
  "utf8"
));
const JOIN_CODE = joinCodes.codes["modern-switching"];
...
test("a student can register, then join a course with the teacher's code", async ({ page }) => {
  ...
  await expect(page.locator(".join-fab")).toBeVisible();
  await page.getByRole("button", { name: /现代交换原理/ }).click();
...
test("waiting for the AI shows the typing dots, and they go away when it answers", async ({ page }) => {
  ...
  await page.locator("#summary-input").fill("交换可以减少终端两两直连所需的线路数量。");
  ...
  await page.locator("#reflect-input").fill("没有交换机时，终端数量增加会让直连线路快速增加。");
...
test("关掉窗口重开是一节新课，按 F5 不是", async ({ page, context }) => {
  ...
  const KEY = "ai-learn.sessionId.modern-switching-week-01";
  ...
  await reopened.getByRole("button", { name: /现代交换原理/ }).click();
...
test("student reaches deep inquiry without entering classroom discussion", async ({ page }) => {
  await registerAndJoin(page, freshNumber());
  ...
  await expect(page.locator("#lesson-title")).toContainText("课程导学与交换的基本概念");
  ...
  await page.locator("#btn-skip-video").click();
  await expect(page.locator("#stage-summary")).toBeVisible();
  await page.locator("#summary-input").fill("交换让多个终端通过共享设备建立连接，而不是两两直连。");
  await page.locator("#summary-submit").click();
  await expect(page.locator("#summary-log [data-typing]")).toHaveCount(0, { timeout: 15_000 });
  ...
  await page.locator("#summary-next").click();
  await expect(page.locator("#stage-reflect")).toBeVisible();
  await page.locator("#reflect-input").fill("如果没有交换机，线路数量会随终端数量呈平方级增长。");
  await page.locator("#reflect-submit").click();
  await expect(page.locator("#stage-reflect")).toBeVisible();
  await expect(page.locator("#stage-discuss")).toBeHidden();
});
