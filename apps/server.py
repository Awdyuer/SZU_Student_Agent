...
def start(body: StartIn) -> dict:
    ...
    with _REGISTRY_LOCK:
        if sid not in SESSIONS:
            ...
            state = restored or initial_state(sid, body.student_id, body.lesson_id)
            SESSIONS[sid] = {
                "state": state,
                "messages": [],
                "lock": threading.Lock(),
        ...
        s = SESSIONS[sid]
    st = s["state"]
    if st.get("lesson_id") not in (None, body.lesson_id):
        raise HTTPException(409, "该会话属于另一节课")
    if st.get("student_id") not in (None, body.student_id):
...
if FRONTEND_OUT_DIR.is_dir():
    # Next.js 使用静态导出后由 FastAPI 同源托管，浏览器无需 CORS 或额外代理。
