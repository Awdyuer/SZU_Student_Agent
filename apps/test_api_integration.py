...
class RecapStrategyTest(unittest.TestCase):
    ...
    lesson_id = "modern-switching-week-01"

    # 第一课复述第一题是 KP-101。测试临时提供三组证据，课程文件本身不保存这些规则。
    FULL = "交换通过共享的中央交换设备连接终端，减少两两直连的线路数量，从而降低组网成本"
    PARTIAL = "交换使用共享交换设备"
    BLANK = "不知道"

    def setUp(self) -> None:
        patcher = mock.patch.dict(agent.EVIDENCE_GROUPS, {
            "KP-101": [["共享", "交换设备"], ["线路"], ["成本"]],
        }, clear=True)
        patcher.start()
        self.addCleanup(patcher.stop)
    ...
    def test_guide_action_reaches_the_model(self) -> None:
        ...
        self.assertTrue(
            any(action in user for user in captured),
            "策略表的「动作」没进模型",
        )
        self.assertTrue(any("【本轮指令】" in user for user in captured))
