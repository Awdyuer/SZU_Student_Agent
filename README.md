...
界面顶部的进度轨始终显示当前处在哪一阶段，跨阶段时有一道全屏毛玻璃过渡。
...
完整的接口契约、请求响应示例、以及后端需要补齐的能力清单，见 **[后端接口说明.md](后端接口说明.md)**。
...
- 掌握度（0–5 星）在前端还没有展示。
- 目前只有一节课，不提供选课；多课程的接口契约写在 `后端接口说明.md` 末尾。
