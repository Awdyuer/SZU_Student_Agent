...
- **课后 · 学习报告**：知识点掌握（0–5 星）、各阶段表现、课后建议。只呈现学生自己的部分，不展示教师侧的会话标识与原始证据摘录
...
- 全部是本地假数据，**没有真实后端联调过**。
- 课后学习报告已实现，但**仍是 mock**：接口层按后端学情导出的真实响应结构预留（`GET /api/session/<sessionId>/export?fmt=json`），`USE_MOCK` 一开就走假报告。联调前后端需要先补 CORS，并让会话 id 与前端对齐（详见[后端接口说明](docs/api/后端接口说明.md)第 9 节）。
- 课后报告只呈现学生自己的部分；教师侧的会话标识、原始证据摘录不展示。
