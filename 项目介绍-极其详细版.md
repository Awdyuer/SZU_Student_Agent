...
Python 依赖非常少：`fastapi`、`uvicorn`、`langgraph`。前端主要依赖 `next`、`react`、`react-dom`。
...
| `lesson_id` | 一节课的稳定主键 | `example-week-1` |
...
上传课时不会混入其它课程的知识库，避免不同课时之间发生题目串课。
...
- `lesson-data/lesson-plan/<lesson_id>.json` 把元数据、阶段和段落放在一个文件中；
...
1. `stages/recap_discussion/questions.md`；
2. `runtime/TMISSION.md` 中的检验问题；
3. `rules/KNOWLEDGE-BASE.md` 中的检测问题。
...
课程流程、问题和模型上下文可以运行，但当前没有内置关键词证据组。若需要稳定自动产生 2～4 星，后续应在课时定义中补充每个知识点的可观察证据。
